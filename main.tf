...
# Router and Cloud NAT for installing packages from repos (apache, php etc)
...
# cloudloadbalancing_ext_http_gce_plus_bucket
module "gce-lb-https" {
  source  = "../../"
  name    = var.network_name
  project = var.project
  ...
  url_map           = google_compute_url_map.tier-network.self_link
...
resource "google_compute_url_map" "tier-network" {
...
//the path in the bucket matches the paths in the url map path rule above.
